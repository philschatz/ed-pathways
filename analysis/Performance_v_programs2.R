...
fit_eng <- lm(Eng_Met ~ PercentEligible, data = overall, weights = Enrollment)
fit_math <- lm(Math_Met ~ PercentEligible, data = overall, weights = Enrollment)
fit_sci <- lm(Sci_Met ~ PercentEligible, data = overall, weights = Enrollment)
...
agg <- melt(agg, id.vars = 1, variable.name = "Subject")
...
g <- g + geom_point(aes(color = Programs))
